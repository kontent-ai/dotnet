...
var fork = Regex.Match(Git(samplesRoot, "remote", "get-url", "origin") ?? "", @"github\.com[:/]([^/]+)/").Groups[1].Value;
...
try
{
    pr = Process.Start(new ProcessStartInfo("gh",
...
if (pr is null)
{
    Console.Error.WriteLine($"sync-code-samples: gh is not available; open the pull request from {fork}:{branch} to {UpstreamRepository}:{targetBranch} yourself");
    return 1;
}
