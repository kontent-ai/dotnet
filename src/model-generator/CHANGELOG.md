...
- Targets `Kontent.Ai.Management` 9.0.0-beta-5. Management model emission follows that SDK's beta-3 rename of `SnippetIdentifier` to `Snippet`, which unified identifier-pair properties on the bare style. The wire format is unchanged.
...
- Restore no longer fails with NU3012. Refit now resolves to 10.2.0 — the 10.1.6 packages are signed with a revoked certificate. This is fixed at the source: `Kontent.Ai.Management` 9.0.0-beta-5 asks for 10.2.0, so no version pin is needed here.
