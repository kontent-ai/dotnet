...
`ResilienceConfiguration` holder - the replacement callback and ordered retry tuning callbacks -
rather than the builder, so the handler chain does not keep the service collection reachable. And for
the default client a change-token source registered under the unnamed name composes the tokens of every source registered
