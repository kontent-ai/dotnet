...
public class EnvelopeConverter_WriteTests
{
    ...
    [Fact]
    public void DateTime_WithZone_WritesDisplayTimezoneSibling()
    {
        var publishedAt = new DateTimeOffset(2024, 6, 1, 12, 0, 0, TimeSpan.Zero);
        var article = new Article { PublishingDate = new DateTimeValue { Value = publishedAt, DisplayTimeZone = "Europe/Prague" } };

        var envelopes = ParseEnvelopes(Converter.WriteEnvelopes(article));

        envelopes["publishing_date"].GetProperty("value").GetDateTimeOffset().Should().Be(publishedAt);
        envelopes["publishing_date"].GetProperty("display_timezone").GetString().Should().Be("Europe/Prague");
    }

    [Fact]
    public void UrlSlug_WritesValueAndMode()
    {
        var article = new Article { Slug = new UrlSlugValue { Value = "on-roasts", Mode = UrlSlugMode.Custom } };

        var envelopes = ParseEnvelopes(Converter.WriteEnvelopes(article));

        envelopes["slug"].GetProperty("value").GetString().Should().Be("on-roasts");
        envelopes["slug"].GetProperty("mode").GetString().Should().Be("custom");
    }

    [Fact]
    public void UrlSlug_FromBareString_IsCustomMode()
    {
        var article = new Article { Slug = "on-roasts" };
    ...
    [Fact]
    public void UrlSlug_Autogenerated_OmitsValueAndWritesAutogeneratedMode()
    {
        var article = new Article { Slug = new UrlSlugValue { Mode = UrlSlugMode.Autogenerated } };

        var envelopes = ParseEnvelopes(Converter.WriteEnvelopes(article));

        envelopes["slug"].TryGetProperty("value", out _).Should().BeFalse("a server-derived slug omits value rather than sending null");
        envelopes["slug"].GetProperty("mode").GetString().Should().Be("autogenerated");
    }

    [Fact]
    public void Custom_WritesValueAndSearchableValueSibling()
    {
        var article = new Article { Rating = new CustomValue { Value = "{\"formId\":42}", SearchableValue = "Almighty form!" } };

        var envelopes = ParseEnvelopes(Converter.WriteEnvelopes(article));

        envelopes["rating"].GetProperty("value").GetString().Should().Be("{\"formId\":42}");
        envelopes["rating"].GetProperty("searchable_value").GetString().Should().Be("Almighty form!");
    }

    [Fact]
    public void Custom_WithoutSearchableValue_OmitsSibling()
    {
        var article = new Article { Rating = "{\"formId\":42}" };

        var envelopes = ParseEnvelopes(Converter.WriteEnvelopes(article));

        envelopes["rating"].GetProperty("value").GetString().Should().Be("{\"formId\":42}");
        envelopes["rating"].TryGetProperty("searchable_value", out _).Should().BeFalse("searchable value is unset");
    }

    [Fact]
    public void Custom_WithNullValue_OmitsValue()
    {
        var article = new Article { Rating = new CustomValue { SearchableValue = "searchable only" } };

        var envelopes = ParseEnvelopes(Converter.WriteEnvelopes(article));

        envelopes["rating"].TryGetProperty("value", out _).Should().BeFalse("a null custom value is omitted rather than sent as null");
        envelopes["rating"].GetProperty("searchable_value").GetString().Should().Be("searchable only");
    }
