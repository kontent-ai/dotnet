...
- **An upload that cannot be re-sent is no longer retried with the wrong body.** `429` is retried for every method, including the upload `POST`, because a rejected request was never processed. But a `FileContentSource` built from a non-seekable stream is consumed by the first attempt, so what went out on the retry was an **empty body** — a different request wearing the first one's clothes. Whether the API stores it or rejects it, the outcome is wrong: either a truncated asset reported as success, or a validation error in place of the `429` that actually happened. Such a request is now left alone and the original failure is returned, so the decision rests with the caller, who is the only one able to rewind the source. Sources created from a `byte[]`, a file path, or a seekable stream re-read cleanly and still retry as before.
