...
- **One builder, two hosting modes**: `services.AddManagementClient(management => …)` (DI, keyed/named clients, options validation) and `ManagementClient.Create(management => …)`, which runs the same registration in a private container the built client owns. The builder is `IManagementClientBuilder` — `Options`, `HttpClient`, `SubscriptionHttpClient`, `TuneRetry`, `ConfigureResilience`, `Services`, `Name`; the shared implementation is `src/common/Clients`. The plain constructor is `Create(options)` and is disposable.
