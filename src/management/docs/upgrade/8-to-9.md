...
```
  ...
  `DisableFor` excludes by method, so it also stops a rate-limited `POST` from being retried, where the built-in rule would have honoured `Retry-After`. See the [README](https://github.com/kontent-ai/dotnet/blob/main/src/management/README.md#resilience-and-the-http-pipeline).
