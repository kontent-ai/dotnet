...
- **Replace the pipeline** with `ConfigureResilience` on the builder, in `AddManagementClient` or `ManagementClient.Create`. It replaces rather than adds, so the idempotency rule above goes with it - re-apply it, or you are back to the 8.x behaviour of replaying writes:
...
```
  ...
  `DisableFor` excludes by method, so it also stops a rate-limited `POST` from being retried, where the built-in rule would have honoured `Retry-After`. See [Retries and timeouts](https://github.com/kontent-ai/dotnet/blob/main/src/management/docs/configuration.md#retries-and-timeouts).
