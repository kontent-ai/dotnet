# Upgrade Guide: Kontent.Ai.Management 8 → 9
...
- [ ] Regenerate strongly-typed models: element properties are now plain values / `*Value` records, and the model is an immutable record.
...
> [!TIP]
> Generate content-type records with the [model generator's Management mode](https://github.com/kontent-ai/dotnet/tree/main/src/model-generator#management-models). For writes without generated models, use the element records in [§5](#5-authoring-elements-without-a-generated-model).
...
- **Tune the default retries** with `management.TuneRetry(retry => retry.MaxRetryAttempts = 5)`, in `AddManagementClient` or `ManagementClient.Create`. Unchanged settings retain their SDK defaults, including the idempotency rule and `Retry-After` handling.
- **Replace the pipeline** with `ConfigureResilience` when you need different strategies or ordering. Replacement drops the idempotency rule and bypasses `TuneRetry`; a bare `HttpRetryStrategyOptions` retries ambiguous writes too. See [Retries and timeouts](https://github.com/kontent-ai/dotnet/blob/main/src/management/docs/configuration.md#retries-and-timeouts) for the configuration contract.
...
| `FilterItemsWithVariantsAsync` | unchanged name — now returns every page | Materialized `IReadOnlyList`; `FilterItemsWithVariantsPageAsync` fetches one page. `9.0.0` shipped it as `ListItemsWithVariantsByFilterAsync`, now obsolete. |
| `BulkGetItemsWithVariantsAsync` | unchanged name — now returns every page | Materialized `IReadOnlyList`; `BulkGetItemsWithVariantsPageAsync` fetches one page. `9.0.0` shipped it as `ListItemsWithVariantsByBulkGetAsync`, now obsolete. |
