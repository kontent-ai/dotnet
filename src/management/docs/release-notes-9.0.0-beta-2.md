...
- **Patch property-name enums renamed.** Each patch domain has its own enum listing the properties it can PATCH (their members differ), but all four were named `PropertyName` in separate namespaces — a collision that was ambiguous in IntelliSense and at call sites. They're now domain-prefixed: `CollectionPropertyName`, `CustomAppPropertyName`, `SpacePropertyName`, `TaxonomyGroupPropertyName`. Members and JSON wire values are unchanged.
