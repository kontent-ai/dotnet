# Upgrade Guide: Kontent.Ai.Delivery 18 → 19

> [!NOTE]
> This guide covers one major: 18.x to 19.x, and describes the 19 API as it shipped through 19.4.0. Moving
> on to 20? Follow [19 → 20](19-to-20.md) next; guides cover one major each and are read in sequence.
...
- [ ] If you used the Sync API (`PostSyncInitAsync`/`GetSyncAsync`/V2), migrate to the standalone [`Kontent.Ai.Sync`](https://github.com/kontent-ai/dotnet/tree/main/src/sync) package
...
var resolver = new HtmlResolverBuilder()
    .WithContentItemLinkResolver("article", async (link, resolveChildren) =>
    {
        ...
        return $"<a href=\"/articles/{HtmlEncoder.Default.Encode(link.Metadata?.UrlSlug ?? "")}\">{innerHtml}</a>";
    })
    .WithContentItemLinkResolver("product", async (link, resolveChildren) =>
    {
        var innerHtml = await resolveChildren(link.Children);
        return $"<a href=\"/shop/{HtmlEncoder.Default.Encode(link.Metadata?.UrlSlug ?? "")}\">{innerHtml}</a>";
    })
    // Global fallback resolver - also where a broken link arrives
    .WithContentItemLinkResolver(async (link, resolveChildren) =>
    {
        var innerHtml = await resolveChildren(link.Children);

        // Metadata is null when the link is broken, which is ResolveBrokenLinkUrlAsync's case
        if (link.Metadata is null)
        {
            return $"<a href=\"/404\">{innerHtml}</a>";
        }

        return $"<a href=\"/content/{HtmlEncoder.Default.Encode(link.Metadata.UrlSlug)}\">{innerHtml}</a>";
    })
    .Build();
...
`ResolveBrokenLinkUrlAsync` has no direct equivalent. `IContentItemLink.Metadata` is `null` when the link
is broken or the item was not included in the response, and the per-type resolvers match on
`Metadata.ContentTypeCodename` - so a broken link never reaches one and always falls through to the global
resolver. Handle it there, as above. Register no global resolver and the SDK emits a placeholder instead;
`ThrowOnMissingResolver(true)` turns that into an exception.

`DefaultResolvers.UrlPatternResolver` substitutes an empty string for every placeholder it cannot fill, so
a broken link under it renders as `/articles/` rather than your 404 route. Wrap it if you need the
distinction:

```csharp
var pattern = DefaultResolvers.UrlPatternResolver(new Dictionary<string, string>
{
    ["article"] = "/articles/{urlslug}"
});

var resolver = new HtmlResolverBuilder()
    .WithContentItemLinkResolver(async (link, resolveChildren) => link.Metadata is null
        ? $"<a href=\"/404\">{await resolveChildren(link.Children)}</a>"
        : await pattern(link, resolveChildren))
    .Build();
```
...
var resolver = new HtmlResolverBuilder()
    .WithContentResolver<Tweet>(tweet =>
        $"<blockquote class=\"twitter-tweet\">" +
        $"<p>{HtmlEncoder.Default.Encode(tweet.Elements.TweetText ?? "")}</p>" +
        $"<cite>@{HtmlEncoder.Default.Encode(tweet.Elements.Handle ?? "")}</cite>" +
        $"</blockquote>")
    .Build();
...
var resolver = new HtmlResolverBuilder()
    .WithContentResolver<Video>(async video =>
    {
        ...
        return $"<div class=\"video-embed\">" +
               $"<iframe src=\"https://youtube.com/embed/{HtmlEncoder.Default.Encode(video.Elements.VideoId ?? "")}\" " +
               $"title=\"{HtmlEncoder.Default.Encode(metadata.Title ?? "")}\"></iframe></div>";
    })
    .Build();
```
...
var resolver = new HtmlResolverBuilder()
    .WithContentResolver("tweet", content =>
    {
        ...
        {
            return $"<blockquote>{HtmlEncoder.Default.Encode(tweet.Elements.TweetText ?? "")}</blockquote>";
        }
        return string.Empty;
    })
    .Build();
    ...
    .WithContentResolvers(
        (typeof(Tweet), content =>
            content is IEmbeddedContent<Tweet> t
                ? $"<blockquote>{HtmlEncoder.Default.Encode(t.Elements.TweetText ?? "")}</blockquote>"
                : ""),
        (typeof(Video), content =>
            content is IEmbeddedContent<Video> v
                ? $"<iframe src=\"https://youtube.com/embed/{HtmlEncoder.Default.Encode(v.Elements.VideoId ?? "")}\"></iframe>"
                : ""),
        (typeof(Quote), content =>
            content is IEmbeddedContent<Quote> q
                ? $"<blockquote><p>{HtmlEncoder.Default.Encode(q.Elements.Text ?? "")}</p><cite>{HtmlEncoder.Default.Encode(q.Elements.Author ?? "")}</cite></blockquote>"
                : ""))
    .Build();
```
...
var resolver = new HtmlResolverBuilder()
    .WithInlineImageResolver((image, resolveChildren) =>
    {
        ...
        return ValueTask.FromResult(
            $"<figure><img src=\"{HtmlEncoder.Default.Encode(image.Url.ToString())}\" alt=\"{HtmlEncoder.Default.Encode(alt)}\" />" +
            $"<figcaption>{HtmlEncoder.Default.Encode(alt)}</figcaption></figure>");
    })
    .Build();
```
...
// You do NOT need to register the source-generated provider manually in the common case — see below
// for when discovery does not reach it.
...
> [!IMPORTANT]
> Order matters only when **overriding** `ITypeProvider`. The SDK registers its default `TypeProvider` via
> `TryAddSingleton`, so a manual `AddSingleton` registered *before* `AddDeliveryClient` will replace it.

**How discovery actually works, and when it misses.** The default provider looks for a type named
`Kontent.Ai.Delivery.Generated.GeneratedTypeProvider` in the entry assembly, then in the assemblies that
entry assembly *directly* references. The `[ContentTypeCodename]` attribute plays no part in finding it;
the source generator emits both, and the lookup is by type name.

Two consequences worth knowing before you debug an empty model:

- **Direct references only.** Models in a project the entry assembly reaches indirectly - a web project
  referencing Services, which references Models - are not found. Reference the models project from the
  entry assembly, or register the generated provider yourself:
  `services.AddSingleton<ITypeProvider, Kontent.Ai.Delivery.Generated.GeneratedTypeProvider>();`
- **Failure is silent.** No provider found means every content type maps to dynamic elements rather than
  your models, which surfaces as properties that are null rather than as an error. The result is cached
  for the life of the process, so this does not resolve itself later.
...
services.AddDeliveryClient(
    ...
    {
        builder.AddHttpMessageHandler<AuditingHandler>();
    });
```

The callback receives the `IHttpClientBuilder` for the SDK's named `HttpClient`, so you can also attach
Polly handlers, configure the primary handler, and so on.

> [!WARNING]
> Do not set `HttpClient.Timeout` here. The SDK sets it to `Timeout.InfiniteTimeSpan` so that its
> resilience pipeline owns timing; a value you set replaces that and bounds the *whole* call, retries and
> backoff included, cutting a pipeline off partway through its last attempt. Bound a single attempt with
> `AddTimeout` inside `configureResilience` instead. Version 20 adds `DeliveryOptions.Timeout` for the
> whole-call ceiling, which outranks the pipeline.
...
| `IDeliverySyncInitResponse`, `IDeliverySyncResponse`, `IDeliverySyncV2Response` | Sync was extracted to the standalone [`Kontent.Ai.Sync`](https://github.com/kontent-ai/dotnet/tree/main/src/sync) package — see [§11](#11-migrating-the-sync-api). |
...
| `PostSyncInitAsync(params)`, `GetSyncAsync(token)`, `PostSyncV2InitAsync()`, `GetSyncV2Async(token)` | Sync API moved to the standalone [`Kontent.Ai.Sync`](https://github.com/kontent-ai/dotnet/tree/main/src/sync) package — see [§11](#11-migrating-the-sync-api). |
...
if (result.IsSuccess)
{
    ...
    while (result.Value.HasNextPage)
    {
        ...
        // null means there was no next page after all; a failed result ends the walk
        if (next is not { IsSuccess: true })
        {
            break;
        }
...
`FetchNextPageAsync()` returns `null` when there is no next page, so assign it to a separate variable and
break on `null` or on failure. Assigning it straight back to `result` and testing only `IsSuccess` leaves
the loop re-reading the previous page's `HasNextPage`, which never advances.
...
A partial walk is indistinguishable from a complete one, so anything that persists what it enumerated -
an export, a search index build - should handle pages explicitly rather than rely on this.
...
> [!NOTE]
> Both of these change in 20. `EnumerateAsync()` throws `DeliveryRequestException` on a failed page rather
> than ending quietly, and `EnumerateItemsWithStatusAsync` is removed in favour of
> `EnumerateAsync().AsPages()`. If you are heading to 20 anyway, see
> [19 → 20 §2](19-to-20.md#2-a-failed-page-throws) before you write this loop twice.
...
<PackageReference Include="Kontent.Ai.Delivery" Version="19.0.0" />
<PackageReference Include="Kontent.Ai.Sync" Version="1.0.1" />
```
...
if (page.IsSuccess)
{
    foreach (var change in page.Value.Items)
    {
        ProcessChange(change);
    }
...
// Or pull several pages at once (maxPages bounds the work per call)
var all = await syncClient.GetAllDeltaAsync(syncToken, maxPages: 10);

if (all.IsSuccess)
{
    foreach (var delta in all.Responses)
    {
    ...
    syncToken = all.FinalSyncToken;
}
```

> [!NOTE]
> The Sync SDK has its own version line and release cadence. The API above is Sync 1.x, which is what ships
> alongside Delivery 19. Sync 2.0 reshapes it - `GetAllDeltaAsync` becomes the `EnumerateDeltaAsync` stream,
> the delta item interfaces become one generic `SyncChange<TData>`, and `InitializeSyncAsync` returns a
> non-generic result. Take that step from the [Sync 1 → 2 guide](../../../sync/docs/upgrade/1-to-2.md), not
> from here.
...
There are two causes, and they look identical from the call site.

**Cause 1: models generated by an older generator.** Regenerate with 10.0.0. Content type models are always
overwritten, so no extra flag is needed:
```bash
dotnet tool update -g Kontent.Ai.ModelGenerator --version 10.0.0
KontentModelGenerator --environmentid <id> --outputdir Models
```

An argument the generator does not recognise fails the run with `Unsupported parameter` and generates
nothing. For Delivery models the accepted set is `--environmentid`, `--outputdir`, `--namespace`,
`--baserecord` and `--nullability`.

**Cause 2: the generated type provider was not found.** The SDK searches the entry assembly and its direct
references only, silently falling back to dynamic elements when it finds nothing. Register it explicitly:
```csharp
services.AddSingleton<ITypeProvider, Kontent.Ai.Delivery.Generated.GeneratedTypeProvider>();
```

Enable logging to tell the two apart: a model that mapped but could not read an element logs a warning
naming the element, while a content type with no model at all logs that it is using `DynamicElements`.
