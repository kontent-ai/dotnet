# Upgrade Guide: Kontent.Ai.Sync 1 → 2
...
`Data` is `required`, deletions included — the API documents it as present on all four delta objects,
and a deletion names what was deleted. If you ported code written against an early `2.0` prerelease,
drop the `?`: `item.Data?.System.Codename` becomes `item.Data.System.Codename`.
