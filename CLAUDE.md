...
- **`publish-batch.yml`** (manual dispatch, after merging that PR) — creates one GitHub Release per product whose declared version is missing from nuget.org, **in dependency order**, waiting for each to reach NuGet before the next. That ordering is a backstop rather than the usual path: floors are raised only after the dependency ships, so a batch's floors already name published versions and any order would do. It matters only if a floor names a version shipping in the same batch — do not arrange that deliberately. Defaults to a dry run; re-running skips what's already published. It decides nothing — the state of `eng/Versions.props` vs nuget.org does.
