...
Guidance for coding agents working anywhere in this repository. `CLAUDE.md` files are one-line pointers to the `AGENTS.md` next to them, so this is the only copy to edit.
...
Products with their own `AGENTS.md` (e.g. `src/management/AGENTS.md`) carry product-specific conventions — read it before working there. A product file holds only what this file cannot say and the code does not make self-evident: architecture pillars, sanctioned divergences from siblings, live-API unknowns. It never repeats repo-wide style, build or release rules. For any product, its sibling products are canonical references: when a style or architecture question comes up, read how a sibling solved it first, and diverge only with a stated reason.
...
- **`prepare-release.yml`** (manual dispatch) — one field per product, taking `prerelease`, `release`, `patch`, `minor`, `major` or an explicit version, empty to skip; it runs `eng/scripts/update-version.cs` to bump `eng/Versions.props` and promote each product's `CHANGELOG.md` `## Unreleased` section, then opens **one PR for the whole batch** against the dispatched branch. It publishes nothing.
- **`release.yml`** (*Publish*, manual dispatch, after merging that PR) — packs and pushes every product whose declared version is missing from nuget.org, and records each as a `<product>-v<version>` tag and a GitHub Release with notes from the changelog. The tag is created *before* the first push so a version is bound to one commit, and the release stays a draft until every package and asset is up; a failed run is fixed by re-running that run, and a fresh dispatch from another commit is refused for that version. Defaults to a dry run that still builds, packs and renders the notes. It **refuses to publish** a product whose changelog has no entry for the version or whose cross-product floor is not on nuget.org (`eng/scripts/dependency-floors.cs` runs as a preflight). It decides nothing — the state of `eng/Versions.props` vs nuget.org does. From any branch other than `main` or `vnext` the products must be named, which is the hotfix path (`maintenance/*` branches). The job runs under the `nuget.org` GitHub environment, which holds the branch policy and any required approval, and the filename is pinned by the nuget.org trusted-publishing policy.
- **A product's first stable release publishes after the products it depends on.** Intra-product references are ProjectReferences and pack at the product's own version, but a cross-product floor is whatever `Directory.Packages.props` names - so publishing `aspnetcore` or `model-generator` in the same wave as `delivery` and `management` would ship a stable package declaring a prerelease dependency. Publish the depended-on products, raise the floors in their own PR, then publish the dependents; `release.yml` takes a product list for exactly this.
